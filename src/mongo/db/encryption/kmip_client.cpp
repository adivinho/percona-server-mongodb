...
public:
    ...
    std::variant<Key, KeyEntryError> getSymmetricKey(const std::string& keyId, bool verifyState);
...
std::variant<Key, KeyEntryError> KmipClient::Impl::getSymmetricKey(const std::string& keyId,
                                                                   bool verifyState) {
...
std::optional<KeyEntryError> KmipClient::Impl::verifyKeyIsActive(const std::string& keyId) {
    auto session = std::make_shared<detail::KmipSessionVerifyKeyIsActive>(keyId);
    conductSession(session);
...
std::variant<Key, KeyEntryError> KmipClient::getSymmetricKey(const std::string& keyId,
                                                             bool verifyState) {
    return _impl->getSymmetricKey(keyId, verifyState);
}

std::optional<KeyEntryError> KmipClient::verifyKeyIsActive(const std::string& keyId) {
    return _impl->verifyKeyIsActive(keyId);
}
}  // namespace mongo::encryption
